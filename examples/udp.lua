--udp 服务：UDP echo（收什么回什么，演示 starnet.socket.udp 监听 + dispatch("udp") + starnet.socket.send_udp 回包）
local starnet = require "starnet"

starnet.start(function()
    print("[lua] udp start id:"..starnet.self())
    --绑定 8003 端口收 UDP 包
    local fd = starnet.socket.udp("0.0.0.0", 8003)
    if fd >= 0 then
...
starnet.dispatch("udp", function(fd, msg, addr, port)
    ...
    --echo：回给发件人
    starnet.socket.send_udp(fd, msg, addr, port)
end)