...
- `skynet_globalexit` / `skynet_context_dispatchall`：优雅全局退出——**建议补**（starnet 现仅 `signal(SIGPIPE, SIG_IGN)`，无 SIGINT/SIGTERM 处理与停机排空；游戏服务器停机保护刚需）
- `skynet_daemon.c`：守护进程化——引擎可做，但 systemd/supervisor 可替代，暂缓
...
| **P6（高级）** | 7. 监视器（✅ 已完成）、集群（cluster ✅ 已补 / harbor 不做）、sharedata（✅ 已补，引擎内精简版 / sharetable、datasheet 待补）、UDP（✅ 已完成）、connect（✅ 已完成）、bind 已有 fd（✅ 已完成）、写缓冲优先级（✅ 已完成）、连接控制（✅ 已完成）、进程级能力（优雅退出建议补，daemon/profile 暂缓）；标准服务集与 lualib → 业务层，见 `business-layer-modules.md` | P4 |
